...
async function fetchDataFromAPIEndpoint() {
    ...
    // console.log(cards.map((card)=>card.properties.Btn_txt.rich_text[0].plain_text))
    ...
    <article class="card">
      ...
      <a href="${card.properties.Link.url}" class="card__btn">${card.properties.Btn_txt.rich_text[0].plain_text}</a>
    </article>
    `
        )
        .join('');
